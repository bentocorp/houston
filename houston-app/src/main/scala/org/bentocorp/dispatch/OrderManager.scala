...
import org.bentocorp.AddOnList.AddOn
...
class OrderManager {
    ...
    def createOrderAheadOrders(scheduledWindowStart: Timestamp, scheduledWindowEnd: Timestamp): MMap[Long, Order[Bento]] = {
        ...
        val orderAddons = orderDao.getOrderAddons(scheduledWindowStart, scheduledWindowEnd)

        // Make an index to store stuff
        val addonIdx = MMap[Long, AddOnList]()

        // Either get from index, or make it for the first time
        orderAddons foreach {
            case (pk_OrderItem, fk_Order, fk_item, qty, name) =>

                val addonList = addonIdx.get(fk_Order) match {
                    case Some(v) =>
                        v
                    case _ =>
                        val newList = new AddOnList()
                        addonIdx += fk_Order -> newList
                        newList
            }

            val addon = new AddOn(fk_item, name, qty)
            addon.setPkOrderItem(pk_OrderItem)
        ...
        print(ScalaJson.stringify(orders))
